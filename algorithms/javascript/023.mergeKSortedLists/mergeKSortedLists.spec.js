import { ListNode, mergeKLists } from './mergeKSortedLists'
...
describe('mergeKLists', function () {
  ...
  it('mergeKLists test case 2', function () {
    const l1 = [].reduceRight(reducer, null)
    const l2 = [1, 3, 4].reduceRight(reducer, null)
    const l3 = [5, 8, 9].reduceRight(reducer, null)
    const result = [1, 3, 4, 5, 8, 9].reduceRight(reducer, null)
    expect(mergeKLists([l1, l2, l3])).toEqual(result)
  })
  it('mergeKLists test case 3', function () {
    const l1 = [].reduceRight(reducer, null)
    const result = [].reduceRight(reducer, null)
    expect(mergeKLists(l1)).toEqual(result)
  })
})
